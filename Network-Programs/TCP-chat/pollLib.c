...
#include "networks.h" /* Added for verbosity flag*/
#include "safeUtil.h"
#include "pollLib.h"
...
void setupPollSet()
{
  ...
  pollFileDescriptors = (struct pollfd *) sCalloc(POLL_SET_SIZE,
...
int pollCall(int timeInMilliSeconds){
  ...
  // If timeInMilliSeconds == 0 it will return immediately after looking at poll set
  
  int i = 0;
  int returnValue = -1;
  int pollValue = 0;
	
  if ((pollValue = poll(pollFileDescriptors,
			maxFileDescriptor,
...
static void growPollSet(int newSetSize){
  ...
  pollFileDescriptors = srealloc(pollFileDescriptors,
				 newSetSize * sizeof(struct pollfd));	
